...
public class Spatula {
    ...
    public Spatula(String name, SpatulaConstants moduleConstants){
        m_moduleName = name;
        m_motor = new WPI_TalonSRX(moduleConstants.spatulaMotorID);
        m_kP = moduleConstants.spatulaP;
        m_kI = moduleConstants.spatulaI;
        m_kD = moduleConstants.spatulaD;
        m_kIZone = moduleConstants.spatulaIZone;
        m_positionAllowedError = moduleConstants.spatulaPositionAllowedError;
        m_positionFullRotation = moduleConstants.spatulaSensorTicks;
        ...
        m_motor.configSelectedFeedbackSensor(  FeedbackDevice.CTRE_MagEncoder_Absolute, // Local Feedback Source
                                            ...
                                            m_motor.configFeedbackNotContinuous(moduleConstants.spatulaSensorNonContinuous, 0); //Disable continuous feedback tracking (so 0 and 1024 are effectively one and the same)
...
*/  
        ...
        m_motor.setSensorPhase(moduleConstants.spatulaSensorPhase); //set the sensor phase based on the constants setting for this module
        m_motor.setInverted(moduleConstants.spatulaIsInverted); //set the motor direction based on the constants setting for this module
